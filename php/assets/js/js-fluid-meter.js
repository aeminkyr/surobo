...
function FluidMeter() {
  ...
  function drawText() {
    ...
    if(options.drawPercentageSign==true){
		var text = 	"%"+currentFillPercentage.toFixed(0) ;

	} else if(options.drawDegreeSign==true) {
		var text = 	currentFillPercentage.toFixed(0)+"c°";
    ...
    init: function (env) {
      ...
      if (env.options) {
        ...
        options.drawPercentageSign = env.options.drawPercentageSign === false ? false : true;
		  options.drawDegreeSign = env.options.drawDegreeSign === false ? false : true;
